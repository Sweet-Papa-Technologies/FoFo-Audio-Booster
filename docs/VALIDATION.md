...
- The final tap initializer explicitly scopes capture to the selected device and output stream; the signed +3 dB/cleanup spike also passes with this route (measured +3.000019 dB). This avoids redirecting a process that targets another device.
...
## Playback continuity regression — September 11, 2026

Version 0.1.1 changes the application discovery/rebuild path. Eight new cases cover master/plugin-only routing under background churn; per-app helper churn; stable solo residual coverage; discovery ordering/names/activity/duplicate IDs; controlled-app relaunch; self-feedback exclusions; relevant output changes; and the actual `AppModel.updateDiscovery` handler. That handler is exercised with 80 background launch/exit cycles and an unrelated connected output: the panel updates without scheduling a rebuild, while controlled-app relaunches still schedule recovery. The complete suite has 22 passing XCTest cases, plus the existing DSP, isolated-worker, four Apple preset, and shader checks.

The earlier 24-hour engine harness finished successfully: 86,401.6 seconds, 288 source relaunches, two externally observed route recoveries, zero reported callback overruns, 0.830% engine CPU and 29.03 MiB maximum RSS; all private objects were released. That harness did not exercise `AppModel`'s discovery/rebuild policy, so it did not detect the unnecessary plugin reloads fixed here. The long run qualifies the engine harness only, not uninterrupted playback through every UI/hardware transition.
## Version 0.1.2 visualizer and app-activity checks

See [0.1.2 changes and measured scope](releases/0.1.2.md). The new analysis regressions cover quiet material, frequency separation, aligned output delay, absent callbacks, reset, and non-finite input. App-activity regressions cover a helper starting playback without a process-list change and retaining idle controls. Live read-only discovery on the development Mac identified the active output apps; the user's exact failing app/scenario was not supplied, so that particular symptom still needs confirmation after updating.
